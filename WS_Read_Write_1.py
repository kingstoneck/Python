...
def on_message(ws, message):
  with open(cfg[usr]['data_file'], 'a') as sd:
    #sd = streaming data
    sd.write("ts:" + str(int(time.time() * 1000)) + "," + message + "\n")
    for line in message:
      line_details = json.loads(message)[0]
      ...
      print("svr_ts:", svr_time, "ts:",timestamp, ",", event, ",", status, ",", message, ",", trade_symbol, ",", trade_id, ",",
            ...
            cursor.rowcount," Record inserted successfully into tsla_test_4 table")
...
def on_error(ws, error):
  with open(cfg[usr]['log_file'], 'a') as sde:
    ...
    sde.write("ts:" + str(int(time.time() * 1000)) + error + "\n")
    for line in error:
...
def on_close(ws):
  with open(cfg[usr]['log_file'], 'a') as sde:
    sde.write("ts:" + str(int(time.time() * 1000)) + ",Websocket Is Now Closed \n")
    print("Websocket Is Now Closed at " + str(int(time.time() * 1000)))

def on_open(ws):
  ws.send(cfg[usr]['wskey'])
  ws.send(cfg[usr]['wssymbols'])
  ...
  if __name__ == "__main__":
    try:
      ...
      connection = mysql.connector.connect(
                                           host=cfg['mysql_test']['host'],
                                           database=cfg['mysql_test']['db'],
                                           user=cfg['mysql_test']['usr'],
                                           password=cfg['mysql_test']['pwd'])
      print("MySQL is connected to", cfg['mysql_test']['db'],"!")
